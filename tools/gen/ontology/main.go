...
const (
	...
	ecorePath = "sysml2/ecore/SysML.ecore"
...
// Ecore:    %s (property multiplicities)
...
func run(root, out string) error {
	...
	features, err := parseEcore(filepath.Join(root, ecorePath))
	if err != nil {
		return err
	}
	properties, classes, err := tables(decls, features)
	if err != nil {
		return err
	}
	source := fmt.Sprintf(template, owlPath, ecorePath, version, commit, version, commit,
...
// parseEcore reads the metamodel's own ecore into a feature table:
// ecoreFeatures[class][feature] is the feature's upperBound attribute, "-1"
// for an unbounded multiplicity. eStructuralFeatures are read under their
// owning eClassifiers element wherever it sits in the package tree.
type ecoreFeatures map[string]map[string]string

func parseEcore(path string) (ecoreFeatures, error) {
	// #nosec G304 -- the checkout to read is named on the command line.
	file, err := os.Open(path)
	if err != nil {
		return nil, err
	...
	for {
		...
		case xml.StartElement:
			switch t.Name.Local {
			case "eClassifiers":
				classes = append(classes, attr(t, "", "name"))
			case "eStructuralFeatures":
				if len(classes) == 0 {
					return nil, fmt.Errorf("%s: eStructuralFeatures outside any eClassifiers", path)
				}
				name := attr(t, "", "name")
				class := classes[len(classes)-1]
				if features[class] == nil {
					features[class] = map[string]string{}
				}
				features[class][name] = attr(t, "", "upperBound")
			}
		case xml.EndElement:
			if t.Name.Local == "eClassifiers" && len(classes) > 0 {
				classes = classes[:len(classes)-1]
			}
		}
	}
	if len(features) == 0 {
		return nil, fmt.Errorf("%s: no eStructuralFeatures found", path)
	}
	return features, nil
}
...
func tables(decls []declaration, features ecoreFeatures) ([]property, []class, error) {
	var (
		properties []property
		classes    []class
	)
	for _, decl := range decls {
		...
		upper, ok := features[defining][name]
		if !ok {
			return nil, nil, fmt.Errorf("%s: no eStructuralFeatures %s.%s in %s",
				decl.iri, defining, name, ecorePath)
		}
