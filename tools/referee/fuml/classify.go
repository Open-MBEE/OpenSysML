...
// report.Bucket is the bucket the class fixes before a run, or false when only a run
// can decide between pass and fail.
func (c Expressibility) Bucket() (report.Bucket, bool) {
	switch c {
	case NotExpressible:
		return report.BucketNotExpressible, true
	case DiffersByDesign:
		return report.BucketDiffersByDesign, true
	}
	return "", false
}
