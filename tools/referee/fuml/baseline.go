...
const BaselinePath = "docs/project/fuml-referee-baseline.json"
...
// are not compared — they are for whoever adjudicates a moved count.
func Reproduces(committed, fresh *Report) error {
	...
	if moved := movedActivities(committed, fresh); len(moved) > 0 {
...
// movedActivities names the activities whose bucket changed, with both buckets.
func movedActivities(committed, fresh *Report) []string {
	was := make(map[string]report.Bucket, len(committed.Activities))
	for _, a := range committed.Activities {
		was[a.Model+"/"+a.Name] = a.Bucket
	}
	var out []string
	for _, a := range fresh.Activities {
		if b, ok := was[a.Model+"/"+a.Name]; ok && b != a.Bucket {
			out = append(out, fmt.Sprintf("%s %s -> %s", a.Name, b, a.Bucket))
		}
	}
	return out
}
