...
func TestRefereeBuckets(t *testing.T) {
	...
	sum := row(t, r, "Sum")
	if sum.Bucket != oreport.BucketPass || len(sum.Reasons) != 0 || sum.Fired != "" || sum.Runs == 0 || !strings.HasPrefix(sum.Status, "complete") {
		t.Errorf("Sum = %+v", sum)
	}
	...
	if raiser.Bucket != oreport.BucketNotExpressible || raiser.Model != ExceptionTestsFile {
...
func TestRefereeNoRecord(t *testing.T) {
	...
	if r.Buckets[oreport.BucketFail] != 1 || len(r.Activities) != 1 {
		t.Fatalf("report = %+v", r)
	}
...
func TestRefereeDisagreement(t *testing.T) {
	...
	r := refereed(t, s, x, Options{Filter: "Sum"})
	got := r.Activities[0]
	if got.Bucket != oreport.BucketFail || strings.Join(got.Expected, ";") != "result = 6" {
		t.Errorf("Sum = %+v", got)
	}
	...
	r = refereed(t, s, x, Options{Filter: "Sum"})
	got = r.Activities[0]
	if strings.Join(got.Expected, ";") != "result = -" || got.Bucket != oreport.BucketFail {
...
func TestRefereeDiffersByDesignIsFixed(t *testing.T) {
	...
	for _, x := range []ExpectedActivity{agreeing, disagreeing} {
		...
		got := r.Activities[0]
		if got.Bucket != oreport.BucketDiffersByDesign || got.Class != "differs-by-design" || got.Runs == 0 {
			t.Errorf("Sum = %+v", got)
		}
