...
func TestRefereePass(t *testing.T) {
	...
	if report.Buckets[oreport.BucketPass] != 1 || len(report.Buckets) != len(oreport.Buckets) {
...
func TestRefereeDiffersByDesignIsNotInferred(t *testing.T) {
	...
	if row := report.Tests[0]; row.Bucket != oreport.BucketFail || row.Row != "" {
	...
	row := report.Tests[0]
	if row.Bucket != oreport.BucketFail || row.Row != TestRows[toolChoice] {
	...
	row = report.Tests[0]
	if row.Bucket != oreport.BucketDiffersByDesign || row.Row != TestRows[byDesign] {
	...
	if row := report.Tests[0]; row.Bucket != oreport.BucketPass || row.Row != TestRows[byDesign] {
...
func TestRefereeTerminateRuns(t *testing.T) {
	...
	row := report.Tests[0]
	if row.Bucket != oreport.BucketPass || row.Runs == 0 || strings.Join(row.Reached, ",") != "S1(entry)::S2(entry)::S2(exit)" {
		t.Errorf("terminate: %+v", row)
	}
	report, _ = refereeFixture(t, terminate, "", []string{"S1(entry)::S2(entry)"}, Options{})
	if row := report.Tests[0]; row.Bucket != oreport.BucketFail {
...
func TestRefereeUnrunnableBucket(t *testing.T) {
	...
	row := report.Tests[0]
	if row.Bucket != oreport.BucketNotExpressible || len(row.Reached) != 0 {
