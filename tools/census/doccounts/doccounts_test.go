...
func TestLibraryTableOmitsEmptyNameLists(t *testing.T) {
	census := fixtures.LibraryCensus{Command: "c", Packages: []fixtures.LibraryPackage{{Name: "P", Declarations: []string{"P::f"}, Evaluated: []string{"P::f"}}}}
...
func TestReadFiguresRejectsAnInvalidCensusFile(t *testing.T) {
	root := t.TempDir()
	writeDoccountsFixture(t, root)
	writeAt(t, root, fixtures.LibraryCensusPath, `{"command":"c","packages":[{"name":"P","declarations":["P::a"],"evaluated":[]}]}`)
...
`)
	...
	writeAt(t, root, fixtures.LibraryCensusPath, libraryCensusFixture)
