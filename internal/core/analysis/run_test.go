...
func TestPerformReturnsWhatTheCallProduced(t *testing.T) {
	...
	double := f.symbol(t, "Double")
	ctx := f.context(t)
	value, _, err := Perform(context.Background(), Default(), Held(ctx), "test::Double", ctx.Schedule(), Budget{}, Auto(),
	...
	_, _, err = Perform(context.Background(), Default(), Held(ctx), "test::Double", ctx.Schedule(), Budget{}, Auto(),
...
func TestPerformReportsARefusal(t *testing.T) {
	...
	_, _, err := Perform(context.Background(), r, Held(ctx), "test::Double", ctx.Schedule(), Budget{}, Auto(),
