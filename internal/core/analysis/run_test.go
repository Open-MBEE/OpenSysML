...
func TestPerformReturnsWhatTheCallProduced(t *testing.T) {
	...
	double := f.symbol(t, "Double")
	ctx := f.context(t)
	value, _, err := Perform(context.Background(), Default(), request(Held(ctx), "test::Double", ctx.Schedule()),
	...
	_, _, err = Perform(context.Background(), Default(), request(Held(ctx), "test::Double", ctx.Schedule()),
...
func TestPerformReportsARefusal(t *testing.T) {
	...
	_, _, err := Perform(context.Background(), r, request(Held(ctx), "test::Double", ctx.Schedule()),
