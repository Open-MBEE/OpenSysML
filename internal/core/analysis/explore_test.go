...
func TestRegistryExploreIsTheRuntimesExploration(t *testing.T) {
	f := parseFixture(t)
	x, err := Default().Explore(context.Background(), f.building(), "test::race", policy(t, "explore"), raceRun(t, f), Budget{})
	if err != nil {
	...
	if _, err = Default().Explore(context.Background(), f.building(), "test::race", runtime.DefaultSchedulePolicy, raceRun(t, f), Budget{}); !errors.Is(err, runtime.ErrNotExploring) {
