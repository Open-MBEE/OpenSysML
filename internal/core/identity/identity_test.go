...
func TestLibraryCatalogNamesEveryNormativeIDAndIsSharedByOverlays(t *testing.T) {
	...
	if !catalog.Normative(el.ID) || !catalog.Normative(el.OwningMembershipID) || catalog.Normative(rdf.EncodeElementID("ScalarValues::Real")) {
		t.Fatal("Normative should know both ids of a library element and no derived one")
	}
	res := resolve.New(idx)
	model := semantics.NewModel(res)
	res.SetModel(model)
