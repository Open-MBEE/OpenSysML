...
// A tool that succeeds without writing an SVG is a failure too.
func TestRenderDiagramToolWroteNoSVG(t *testing.T) {
	dir := t.TempDir()
	withoutDiagramTools(t)
	fakeTool(t, dir, "dot", DotEnv, "exit 0\n")
	blocks, err := parseBlocks(dotMarkdown)
	if err != nil {
		t.Fatalf("parseBlocks: %v", err)
	}
	...
	var docErr *Error
	if !errors.As(err, &docErr) || docErr.Kind != ErrorToolFailed || !strings.Contains(docErr.Detail, "wrote no SVG") {
		t.Fatalf("got %v, want ErrorToolFailed for a missing SVG", err)
	}
}
