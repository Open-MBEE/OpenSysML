...
func TestRoutingHonorsTheSelectedVariantConnection(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		if err := ctx.postVia(nil, conns, Message{SignalType: "Ping"}, lower.Send{Target: "outPort", IsVia: true}, nil, nil); err != nil {
...
func TestRoutingIsPerOwnerVariantSelection(t *testing.T) {
	...
	for usage, want := range map[string]string{"test::alpha": "inPort", "test::beta": "bypass"} {
		...
		if err := ctx.postVia(nil, nil, Message{SignalType: "Ping"}, lower.Send{Target: "outPort", IsVia: true}, self, nil); err != nil {
			t.Fatalf("%s: %v", usage, err)
		}
		var got []string
		for _, msg := range ctx.PendingMessages() {
...
func TestAddressedSendStaysWithinTheSendingObject(t *testing.T) {
	...
	if err := ctx.post(nil, nil, Message{SignalType: "Ping"}, send, alpha, nil); err != nil {
		t.Fatalf("post: %v", err)
	}
	pending := ctx.PendingMessages()
	if len(pending) != 1 {
...
func TestAddressedSendResolvesPortOfNamedObject(t *testing.T) {
	...
	if err := ctx.post(nil, nil, Message{SignalType: "Ping"}, send, beta, nil); err != nil {
		t.Fatalf("post: %v", err)
	}
	pending := ctx.PendingMessages()
	if len(pending) != 1 {
...
func TestAddressedSendToUnreachablePortIsTyped(t *testing.T) {
	...
	err := ctx.post(nil, nil, Message{SignalType: "Ping"}, send, instanceOfUsage(t, ctx, idx, "test::alpha"), nil)
	if !errors.Is(err, ErrUnroutableSend) {
...
func TestAddressedSendToQualifiedNameFromNoObjectIsDelivered(t *testing.T) {
	...
	if err := ctx.post(nil, nil, Message{SignalType: "Integer"}, send, nil, nil); err != nil {
		t.Fatalf("post: %v", err)
	}
...
func TestAddressedSendToAnObjectNeedsThatObject(t *testing.T) {
	...
	if err := ctx.post(nil, nil, Message{SignalType: "Integer"}, send, alpha, nil); err != nil {
		t.Fatalf("post: %v", err)
	}
	got := ctx.PendingMessages()[0]
...
func TestAddressedSendToReceiverOfAnotherObjectCarriesItsIdentity(t *testing.T) {
	...
	if err := ctx.post(nil, nil, Message{SignalType: "Integer"}, send, beta, nil); err != nil {
		t.Fatalf("post: %v", err)
	}
	got := ctx.PendingMessages()[0]
...
func TestAddressedSendReportsWhyTheObjectCouldNotBeBuilt(t *testing.T) {
	...
	ctx.maxSteps = 0
	err := ctx.post(nil, nil, Message{SignalType: "Ping"}, send, nil, nil)
...
func TestAddressedSendToQualifiedElementOfATwinObject(t *testing.T) {
	...
	for _, tc := range []struct{ target, port, receiver string }{
		...
		ctx.messages = nil
		if err := ctx.post(nil, nil, Message{SignalType: "Integer"}, lower.Send{Target: tc.target, Scope: scope}, beta, nil); err != nil {
