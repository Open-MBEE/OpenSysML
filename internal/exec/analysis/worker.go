...
func (m *Model) WorkerAt(job int) (*Worker, error) {
	...
	slot.worker = &Worker{Model: model, Warming: time.Since(started)}
	return slot.worker, nil
}
...
// limits, seed and clock untouched, else one of the run's own under the budget drawing from
// seed under the draw policy, on a clock stepping by step.
func (m *Model) running(engine string, budget Budget, seed ModelSeed, draws runtime.DrawPolicy, step float64) (*runtime.Context, error) {
	if m.holds() {
		ctx, err := m.Context()
	...
	return Question{ModelSeed: seed, Draws: draws, ClockStep: step}.fresh(m, 0, budget)
}
