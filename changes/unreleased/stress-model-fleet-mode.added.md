- **The satellite-network stress model can be generated as a fleet.** `cmd/stress-model -fleet`
  ...
  requirements, state machine, ring, inter-plane and downlink connectors are unchanged. `-stats`
  now reports the spacecraft definitions and the units carrying values of their own in both forms,
  so the two can be compared: at 12 800 satellites the fleet declares 11 667 elements against
  2 354 827, and validates in 0.57 s and 185 MB rather than 301 s and 20.1 GB. A guide chapter,
