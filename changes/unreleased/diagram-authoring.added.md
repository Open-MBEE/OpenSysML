- **The VS Code diagram panel edits the model.** An **Add…** menu on the panel and a right-click
  ...
  actions, control nodes and successions, a tree every kind the language has. Every action is a
...
- **`opensysml/applyModelEdit` LSP request.** Turns a list of model operations (`setValue`,
  ...
  `palette` naming the member and connection kinds a diagram of that kind offers.
