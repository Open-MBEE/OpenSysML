- **The SysML v1 migration reads past the UML metaclass where the tool's own encoding hides the
  ...
  beside an opaque id (`referentPath`); the tool library's `float`, `double`, `int`, `long`,
  ...
  only when it parses as v2 and every name it uses is visible where it is written, so a
  JavaScript body or a bare enumeration literal stays a comment. An instance of a value type is
  ...
  value of a unique feature, a feature of another classifier — is left as a comment; a real
  ...
  default naming an instance of a block types the usage by that individual instead of being
  written as a value. An undirected part or item property of an interface block is a `ref`,
  since a port owns no composite parts, and a specializing block's property named like an
  inherited one redefines it. Migrating the current TMT observatory model now yields notation
