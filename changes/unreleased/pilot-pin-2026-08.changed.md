- **The pinned OMG pilot implementation is now release `2026-08` (`jupyter-sysml-kernel` 0.62.0)**,
  ...
  beside the installed copy and swapped in only once complete, so a failed clone or Maven run
  keeps the previous validator usable; the SysML, KerML and evaluator builds always go through
  that check before compiling against the jar.
