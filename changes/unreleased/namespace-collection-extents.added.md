- **A namespace-level object usage of several occurrences denotes its objects.** `part wheels :
  ...
  a `[0..*]` usage still denotes none. `all Wheel` counts them beside the `[1]` usages, the usage
  reads as the sequence (or set) of those objects, and `wheels#(1)`, `wheels.radius` and
  `size(wheels)` read them, in the REPL and over gRPC alike; the trace names each member after
  its usage (`wheels #1`, `wheels #2`). It used to denote nothing, so `all Wheel` was refused with
