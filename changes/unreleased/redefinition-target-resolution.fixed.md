- **A redefinition's target is resolved from the owning type's generals and then the enclosing
  ...
  alias it declared (`alias y for x;`), and `:>> C::nope` started at a sibling `C`; the pinned
  OMG pilot leaves all four unresolved, and so does OpenSysML now, so the downstream reports
  those bindings drew (a featuring-type conflict, a conformance error) no longer appear. A
  target the generals lack is still found in the enclosing namespace and outward, as before.
