- **The language server runs the behavior a diagram draws.** The new `opensysml/debug/*`
  ...
  declarations the run was built from — the target's, its performer's, and those of the definitions
  the target takes content from — keeps it running and reports the snapshot in the fresh IDs
  through the new `opensysml/debugChanged` notification, while one that rewrites or removes any of
  them, or the declared view, ends it and says why. To place runtime state on a rendering, the runtime
  ...
  runtime model over a workspace's documents; a lowered `StateGraph` or `ActionGraph` lists the
