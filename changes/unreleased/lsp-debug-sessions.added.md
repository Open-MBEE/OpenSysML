- **The language server runs the behavior a diagram draws.** The new `opensysml/debug/*`
  ...
  queued to be lost. A session follows the document: an edit that leaves the target's declaration
  as it was keeps it running and reports the snapshot in the fresh IDs through the new
  `opensysml/debugChanged` notification, while one that rewrites or removes the target, its
  performer or the view ends it and says why. To place runtime state on a rendering, the runtime
  ...
  runtime model over a workspace's documents. See [the LSP reference](docs/reference/lsp.md).
