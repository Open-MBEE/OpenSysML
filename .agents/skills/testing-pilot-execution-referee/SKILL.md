...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  ...
  diagnostic(s), 26 only ours, 600 only the pilot's` after the expressions example joined `examples/` — read it from the baseline JSON, not from this line, since each
