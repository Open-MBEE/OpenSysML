...
- **Additivity.** `go run ./cmd/pilot-diff` must still print the headline the
  committed baseline holds (`376 file(s), 346 fully agreeing; 38 agreed
  diagnostic(s), 38 only ours, 1126 only the pilot's` after the instance-layer self-model round at the `2026-08` pin — read it from the baseline JSON, not from this line, since each
