...
- Measured after the MOSA example joined `examples/`, with a fresh library cache: `370 file(s), 338 fully agreeing; 34 agreed,
  26 only ours, 1040 only the pilot's`, JSON totals `openSysMLDiagnostics 63 / pilotDiagnostics
  1077 / severityMismatch 3`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the MOSA library round it **is**
current: a live run gives `370 file(s), 338 fully agreeing; 34 agreed, 26 only ours, 1040 only the
