...
- Measured after the Legend of the Red Dragon completed-mechanics round at the `2026-08` pin, with a fresh library cache: `379 file(s), 347 fully agreeing; 38 agreed,
  38 only ours, 1334 only the pilot's`, JSON totals `openSysMLDiagnostics 79 / pilotDiagnostics
  1375 / severityMismatch 3`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the Legend of the Red Dragon completed-mechanics round it **is**
current: a live run gives `379 file(s), 347 fully agreeing; 38 agreed, 38 only ours, 1334 only the
