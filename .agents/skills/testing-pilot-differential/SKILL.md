...
- Measured after the value uniqueness and analysis framework rounds, with a fresh library cache: `370 file(s), 337 fully agreeing; 34 agreed,
  28 only ours, 671 only the pilot's`, JSON totals `openSysMLDiagnostics 64 / pilotDiagnostics
  707 / severityMismatch 2`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the value uniqueness and analysis framework rounds it **is**
current: a live run gives `370 file(s), 337 fully agreeing; 34 agreed, 28 only ours, 671 only the
