...
- Measured after the instance-layer self-model round at the `2026-08` pin, with a fresh library cache: `376 file(s), 346 fully agreeing; 38 agreed,
  38 only ours, 1126 only the pilot's`, JSON totals `openSysMLDiagnostics 79 / pilotDiagnostics
  1167 / severityMismatch 3`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the instance-layer self-model round it **is**
current: a live run gives `376 file(s), 346 fully agreeing; 38 agreed, 38 only ours, 1126 only the
