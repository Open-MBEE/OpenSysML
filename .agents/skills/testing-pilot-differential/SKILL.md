...
- Measured after the runtime showcase round, with a fresh library cache: `375 file(s), 338 fully agreeing; 34 agreed,
  38 only ours, 1118 only the pilot's`, JSON totals `openSysMLDiagnostics 75 / pilotDiagnostics
  1155 / severityMismatch 3`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the runtime showcase round it **is**
current: a live run gives `375 file(s), 338 fully agreeing; 34 agreed, 38 only ours, 1118 only the
