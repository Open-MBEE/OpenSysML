...
- Measured after the Legend of the Red Dragon example round at the `2026-08` pin, with a fresh library cache: `378 file(s), 346 fully agreeing; 38 agreed,
  38 only ours, 1247 only the pilot's`, JSON totals `openSysMLDiagnostics 79 / pilotDiagnostics
  1288 / severityMismatch 3`; ~2 min wall, byte-identical across runs *and* after a from-scratch
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the Legend of the Red Dragon example round it **is**
current: a live run gives `378 file(s), 346 fully agreeing; 38 agreed, 38 only ours, 1247 only the
