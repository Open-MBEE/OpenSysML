...
- Measured after the value uniqueness round, with a fresh library cache: `370 file(s), 337 fully agreeing; 34 agreed,
  28 only ours, 600 only the pilot's`, JSON totals `openSysMLDiagnostics 64 / pilotDiagnostics
...
but only while the baseline is current. Check that first. As of the rebaseline that came with the value uniqueness round it **is**
current: a live run gives `370 file(s), 337 fully agreeing; 34 agreed, 28 only ours, 600 only the
