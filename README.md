...
| Julia client package | ✅ Thin Connect-JSON client over HTTP.jl + JSON.jl (no protobuf, no generated code): parse, diagnostics, symbols, evaluation, instantiation, action and state execution, OSLC query, and `call`/`callRaw` reaching every RPC; the conformance runner drives all 134 scenarios ([client/julia/OpenSysML/README.md](client/julia/OpenSysML/README.md)) |
| MATLAB/Octave client package | ✅ The same thin Connect-JSON client for MATLAB R2019b+ and GNU Octave 7+ — `matlab.net.http` under MATLAB, a `curl` subprocess under Octave, which cannot spawn a private child without Java ([client/matlab/README.md](client/matlab/README.md)) |
