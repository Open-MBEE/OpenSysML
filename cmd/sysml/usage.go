...
func registerFlags(fs *flag.FlagSet) {
	...
	fs.Var(&modelChecks.actions, "action", "Run this action to completion, as -action \"Drive rover1\" to run it on an object; under -schedule explore each run performs it on an object of its own, named as a definition or usage to create, a path into one such as mission.rover, or a run's -instantiate object (repeatable)")
