...
func registerFlags(fs *flag.FlagSet) {
	...
	fs.Var(&jobsFlag, "jobs", "Runs of one check, exploration or sweep that may go concurrently, each on a worker of its own over the shared model; the result is the same at any count. Default OPENSYSML_JOBS, else the number of CPUs")
