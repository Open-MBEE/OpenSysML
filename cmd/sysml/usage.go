...
func registerFlags(fs *flag.FlagSet) {
	...
	fs.Var(&modelChecks.checker.diverge, "check-diverge", "With -engine check: report this feature divergent when schedules leave it with different final values, as -check-diverge x or -check-diverge this.level for the performing object's; default the action's own attributes (repeatable)")
	fs.Var(&modelChecks.checker.properties, "check-property", "With -engine check: evaluate this constraint or requirement at every stable state of the action, on the performing object when there is one, and report a schedule at which it is false (repeatable)")
	fs.StringVar(&modelChecks.checker.witness, "check-witness", "", "With -engine check: write a witness file to this directory for each violation and each divergent value — the schedule's choices, a blank line, then the run's trace — which -schedule replay:<file> and %replay follow")
	modelChecks.checker.depth.flag, modelChecks.checker.states.flag = "check-depth", "check-states"
	fs.Var(&modelChecks.checker.depth, "check-depth", "With -engine check: the most moves one schedule may make before the search backtracks, named as the depth bound hit (default 10000)")
	fs.Var(&modelChecks.checker.states, "check-states", "With -engine check: the most distinct states the search may visit, named as the states bound hit; the same figure -engine all gives an exploration as its runs (default 1000000)")
	fs.Var(&modelChecks.checker.timeout, "check-timeout", "With -engine check: the time the check's plan may run for, as 30s or 2m; a search it stops is reported incomplete with the states and depth reached, not as a verdict")
