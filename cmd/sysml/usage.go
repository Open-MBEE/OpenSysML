...
func doc() usage.Doc {
	return usage.Doc{
		...
		Sections: []usage.Section{{
			...
			Title:      "Environment",
			ManOnly:    true,
			Items:      append(append(usage.BudgetEnvironment(), usage.ToolEnvironment()...), solverEnvironment()...),
...
func registerFlags(fs *flag.FlagSet) {
	...
	fs.Var(&engine, "engine", "Analysis engine every check is put to: auto (default) picks the strongest engine covering the question, all puts it to every covering engine in name order and composes their answers, or an engine by name (run, explore, sweep, solve, or tool:<name> from OPENSYSML_TOOLS), whose refusal is then the answer; -engine explore is -schedule explore")
