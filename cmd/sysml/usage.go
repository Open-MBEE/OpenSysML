...
func registerFlags(fs *flag.FlagSet) {
	...
	fs.Var(&modelChecks.checker.inputs, "check-input", "With -engine smt or -engine all: leave this feature of the action or its performing object free in its declared domain although the model binds it, as -check-input inletTemp; a name that is not such a feature is refused. Without it, every input the model leaves unbound is free and every bound one is pinned (repeatable)")
