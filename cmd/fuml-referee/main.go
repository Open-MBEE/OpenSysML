...
import (
	...
	"github.com/Open-MBEE/OpenSysML/internal/fuml"
)
...
func run(out, log io.Writer, o options) error {
	...
	x, err := fuml.ReadExpected(filepath.Join(repo, filepath.FromSlash(fuml.ExpectedPath)))
	if err != nil {
		return err
	}
	...
	committed := filepath.Join(repo, filepath.FromSlash(fuml.BaselinePath))
	if o.update {
