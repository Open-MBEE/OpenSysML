...
| [lord-demo/lord.sysml](lord-demo/lord.sysml) | [lord-demo/README.md](lord-demo/README.md) | *Legend of the Red Dragon*, the BBS door game, as a system: the town, the shops' stock and the masters as parts; a warrior whose day is a state machine driven by the menu's signals, each place's deed an action the transition performs; a fight whose dice are a decision the schedule resolves, checked over every schedule for the one that kills the warrior and replayed from its witness; `satisfy` of who may face the dragon; the solver on what a first day affords, why a Long Sword is out of reach, and the fewest hit points that win the brawl; the price lists and the warrior's standing in a generated document |
