...
prints the current ones. State of the 215 committed cases, the original 32, the 62 the
...
the 27 of `scalar_classification.cases`, the 24 of `literal_types.cases` and the 6 of
`extent_expressions.cases`:

```
agree: 125 · kind-only: 1 · order-only: 0 · disagree: 7
pilot-unevaluated: 63 · pilot-silent: 7 · pilot-error: 2 · ours-error: 2 · both-error: 8
nondeterministic: 0
```
