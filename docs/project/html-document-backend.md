...
HTML of `internal/doc/docpdf` (see [Rendering a document as HTML](../reference/cli.md#rendering-a-document-as-html)
...
and Prince — and it is built the long way round: `internal/doc/docpdf/markdown.go` re-parses
docrender's Markdown back into flat presentation blocks (heading, paragraph, caption, table,
list, mermaid, anchor) and `internal/doc/docpdf/html.go` writes those blocks as a page with an
...
Markdown, so all three engines keep working. That deletes `internal/doc/docpdf/markdown.go`,
