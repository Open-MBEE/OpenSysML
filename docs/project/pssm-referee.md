...
classification and the join incoming-effects fix described below, as
...
The previous baseline (develop `e6218449a` with the completion-choice fix and the guard
classification, 2026-09-15) counted 43 `pass` and 17 `fail`. One change moves it: a join now fires every transition into it — each source exited, then that
...
One failure cites a note row through the committed table. The other fifteen are
