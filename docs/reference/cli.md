...
without Mermaid diagrams needs no diagram tool. A diagram block with `form = "dot"` is not drawn:
the PDF keeps its DOT source under a notice saying so, and no Graphviz tool is looked for.
