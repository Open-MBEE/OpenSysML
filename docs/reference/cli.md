...
| `-draws <policy>` | How every run the invocation makes resolves the draws of `RandomFunctions` — `uniform`, `uniformInteger`, `triangular`, `normal`: `random` (the default) draws each call from `-seed`; `min`, `max` and `average` take each call's least, greatest or mean value instead and need no seed; `normal` has no least or greatest value, so a run that calls it under `min` or `max` stops with an error. Weighted decisions are not durations: they draw from `-seed` under every policy, and unseeded take their most probable branch. Every witness records the policy as `draws by <policy>`, and `-schedule replay:<file>` follows it. See [Running an action many times](#running-an-action-many-times) |
...
mean `3.0`, `normal(m, s)` is `m` under `average` and has no least or greatest value, so a run
that calls it under `min` or `max` stops with a typed error naming the call. A run whose only
...
is undecided saying so, with the sidecar's notes. The statistics are nearest-rank on both sides,
