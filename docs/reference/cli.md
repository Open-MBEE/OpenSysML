...
| `--from <format>` | | Input format for `--convert`: the `--convert` formats, or `xmi`/`uml` for a SysML v1 model to migrate (default: from the input's extension; `.xmi` and `.uml` are recognized) — see [SysML v1 migration](sysml-v1-migration.md) |
