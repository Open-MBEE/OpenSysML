...
hex), and a stem Windows reads as a device (`CON`, `NUL`, `COM1`, …) has its first byte encoded
too. The rule is the same on every platform and reverses to the view name, so
`Views::'Pointing w/NSEN'` is written as `Views.Pointing w%2FNSEN.mmd`. A name that would run past
...
name no longer reads back to the view name. Two views whose names meet in the same path stop the
run with status 2 naming both. With no `-render-form`,
graph-shaped kinds use Mermaid (`.mmd`) and tables use Markdown (`.md`); a forced text form uses
