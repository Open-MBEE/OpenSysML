...
| `-schedule <policy>` | The scheduling policy every run this invocation starts — `-action`, `-state`, `-analysis`, and a `-calc` whose body performs actions — resolves its [choice points](../guide/06-behavior.md) under: `reverse` (the default: reverse token order, first holding guard, first enabled transition), `declared` (spawn and declaration order) or `seed:<n>` (a pseudo-random order the non-negative integer `n` fixes, the same on every platform). The `took …` in each reported choice is what the policy took. A spelling naming no policy — an unknown name, `seed` or `seed:` without a number, `seed:-1`, `seed:abc` — is refused before anything runs |
