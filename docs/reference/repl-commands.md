...
changes nothing about the debuggers: `%action` under any other engine, `%step` and `%continue`
step one run as before, `%schedule explore` and `%engine explore` are refused at the prompt as
