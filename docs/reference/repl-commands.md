...
| `%check-diverge [<feature>...\|off]` | Show or set the features whose final values the `check` engine compares across schedules — `x` for the action's attribute, `this.level` for the performing object's; `off` (the default) compares every attribute of the action and of its performing object, so an action run without one is compared on its own attributes |
...
| `%action <name> [<object>]` | Start an action debugging session, optionally performed by an instantiated object, given as an [object reference](#object-references) (`%action tally car`, `%action tally #3`). Under `%engine check` the action is checked instead of stepped ([Checking every schedule](#checking-every-schedule-of-an-action)) |
...
the session, and `%replay` steps the run a witness records:
...
changes nothing about the debuggers: `%action` under any other engine, `%step`, `%continue` and
`%schedule explore` step one run as before, `%engine explore` is refused at the prompt as it
was, and a declaration unrelated to a running debugger still leaves it running. `%replay` under
