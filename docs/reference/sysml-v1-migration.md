...
| built-in and value-property columns interleaved (`name`, `mass`, `qualifiedName`) | one `columns = (PropertyColumn(name = "name"), Column(…), PropertyColumn(name = "qualifiedName"))` in column order — `PropertyColumn` reads the built-in property as `properties` does; `properties` is kept when every built-in column precedes the value properties. Column names are unique: the built-in properties claim theirs first, and a value property captioned like one (`Pump::name`) is written `name 2` with the note |
...
| `TableStructure` with `TableAttributeColumn` (`Name`, `Documentation`), `TablePropertyColumn` (a value property of the rows' definition), `TableExpressionColumn` naming a bare query property | `part table : Table { attribute redefines caption = …; calc rows : …; }` over `Project(properties, columns = (Column(…)))` in column order, a built-in property among value properties as a `PropertyColumn`, a value property captioned like a built-in property as `<caption> 2`; `includeDoc` adds `documentation` last; a column beyond these is omitted with the note, and a table with no writable column is refused |
