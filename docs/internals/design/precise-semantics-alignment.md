...
`dispatchEvent`. **agrees**, with one proviso on the order among completion events: the runtime
queues the completions a move generates once the move has settled, leaf by leaf in region
declaration order (`scheduleTransitionEvents`), so the IDs follow the declaration and not the
entry draw (SM22) that decided which state was entered first. Under `declared` the two orders
coincide; under `seed:<n>` and `explore` a completion generated by the region drawn second can
be dispatched before the one drawn first, which PSSM's pool never does. Finding 11 below records
it and the change it belongs to.
...
11. **A completion's firing is not drawn against the entry front — and should not be.** Found
    ...
    *Junction 005*, eight and thirty-one on the History pair), and no narrower rule reaches the
    ...
    - *History 001-C*'s first half is about the **pool's order** (SM10's proviso): PSSM
      ...
      does not follow. A runtime gap, small, a divergence under `reverse`, `seed:<n>` and
      `explore` alone, that on its own moves no test; it is recorded in the design note for a
      runtime change of its own, which completes *Transition 017*'s reachable set now that
      item 9's fourth site draws its do step.
    ...
    - *Terminate 002*'s one remaining trace has this shape with a **do step** in place of the
      ...
      the dispatch alone; it stays `fail` citing this item for it. *Transition 017*'s three are
      the pool's order, beside the suite defect of its two anomalous traces.

    No default golden, `check` verdict or exploration model changes under this item. The five
...
of its four sites, the fourth in the change set that drew the do step against the dispatch; the
pool's order of item 11 and the do step against a sibling's entry unit wait for runtime changes
of their own.
