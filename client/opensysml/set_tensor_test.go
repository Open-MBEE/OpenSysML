...
func TestSetsAndTensorsCrossEveryTransport(t *testing.T) {
	...
	for name, client := range map[string]opensysml.Client{
		...
		t.Run(name, func(t *testing.T) {
			...
			// A set listing an element twice is a failure, not a set of one.
			_, err = client.EvaluateCalc(ctx, model, "W::sizeOf", opensysml.Set{opensysml.Int(1), opensysml.Int(1)})
			var failure *opensysml.FailureError
			if !errors.As(err, &failure) || !strings.Contains(failure.Error(), "set element is repeated") {
				t.Errorf("sizeOf({1, 1}) err = %v, want a failure naming the repeated element", err)
			}
			// So is a tensor its components do not fill.
