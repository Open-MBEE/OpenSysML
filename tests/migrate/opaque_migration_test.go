...
func TestSwimlaneBodiesAndGuardsRunAgainstTheRepresentedPart(t *testing.T) {
	...
	wantNote(t, r, "_drop_o", migrate.Mapped, "from the start of 'first attempt' to the start of (_drop) is assigned to the attribute Time_Drop")
	wantNote(t, r, "_drop", migrate.Approximated, "no edge leaves the node, so the token it takes ends there, as at done")
	wantNote(t, r, "_dropped", migrate.Unmapped, "the observation reads the clock at the end of (_drop), which is no action and so has no end of its own")
	...
	// attempt reaches, hold the last pass.
	for _, want := range []string{
		...
		"Time_Drop: 5 run(s), min 10.0, mean 10.0, max 10.0",
