...
// one without a Mean, one with a fractional N and one holding no t. A second
// configuration stores nothing, a third targets an analysis binding no feature.
func TestMonteCarloAnalysisSnapshotsAreSummaries(t *testing.T) {
	...
	want := []simresults.ConfigurationResults{{
		...
		Notes: []string{
			"1 snapshot(s) hold the MonteCarloAnalysis::Mean as u and not as t, which the analysis binds it to, so they are of an analysis of another configuration and not among the results",
	...
	if values := group0.Values("t"); !reflect.DeepEqual(values, []float64{2, 6, 4}) {
		t.Errorf("Values(t) = %v, want the runs stored one by one, without the summary's mean", values)
	}
	if runs := group0.StoredRuns(); runs != 8 {
		t.Errorf("StoredRuns = %d, want 4 summarised and 4 stored one by one", runs)
	}
	if got, want := r.Results.Summary(), "results of 3 run configuration(s): 2 with 6 stored snapshot(s) standing for 9 run(s)"; got != want {
		t.Errorf("Summary = %q, want %q", got, want)
	}

	wantLine(t, r.Notation, "/* results of the simulation tool: 5 snapshot(s) in Results standing for 8 run(s) analysing t holding p, t */")
