...
func TestConvertFromXMI(t *testing.T) {
	...
	turtle, _, err := convert.Migrate("vehicle.xmi", data, convert.FormatTurtle)
	if err != nil {
	...
	if _, _, err := convert.Migrate("vehicle.xmi", data, convert.FormatXMI); !errors.As(err, &notWritable) {
